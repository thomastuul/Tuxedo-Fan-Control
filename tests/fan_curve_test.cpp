...
int main() {
  ...
  expectEqual("table lower boundary", 26,
              calculateDynamicFanSpeed(FAN_TABLE_MIN_TEMP));
  ...
  expectEqual("table upper boundary", FAN_MAX_SPEED,
              calculateDynamicFanSpeed(FAN_TABLE_MAX_TEMP));
  expectEqual("temperature above table", FAN_MAX_SPEED,
