...
int calculateDynamicFanSpeed(int temperature) {
  const int DUTY_PERCENT = dutyPercentForTemperature(temperature);
  return static_cast<int>(std::round(static_cast<float>(DUTY_PERCENT) /
                                     FAN_MAX_DUTY_PERCENT * FAN_MAX_SPEED));
}
